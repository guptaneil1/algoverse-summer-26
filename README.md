...
| Command | Purpose |
|---|---|
| `make lint` | ruff over the research package, tests, and scripts |
| `make test` | unit and contract tests |
| `make smoke` | toy CPU chain, no model or dataset download |
| `make audit` | required-path and schema audit |
| `make preflight CONFIGS="configs/experiment/*.json"` | budget equality **before** any chain launches |
| `make validate RUN=runs/<run_id>` | classify a completed run `valid` / `valid_with_limitation` / `invalid` (exit 0 / 2 / 1) |
| `make figures` | fixture figures (needs the `figures` extra) |
| `make fixture-artifacts` | every fixture artifact plus a combined SHA-256 manifest |
| `bash scripts/verify_clean_install.sh` | full install and gate check in a throwaway virtualenv |
| `make reproduce-headline` | guarded; exits 3 until a results freeze exists |

Figure generation needs an optional dependency that the core package deliberately omits:

```bash
python -m pip install -e ".[figures]"
```

**Everything under `results/` is fixture output.** It is generated by a simulator that trains no language model, is watermarked accordingly, and may not be cited. See [`results/README.md`](results/README.md).
