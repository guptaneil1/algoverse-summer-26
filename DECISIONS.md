...
**P-012 is not of the same kind and should not be read as part of that block.** Its
substance — spend the remaining compute on the full five-seed grid — is a decision the
owner made directly, on 2026-08-19, against a costed set of options. Its *ordering* clause
is the assistant's, and was unvalidated when written: no phase had run. That clause is a
scheduling choice with no scientific content, changing which chains run first and never
which run or what is measured. It is recorded separately so no reader is told the owner
accepted a proposal when what the owner did was choose how much money to spend.
...
| P-012 | 2026-08-19 | Owner (spend), Assistant (order) | **Finish the interrupted v2 grid to all five seeds, running the seed blocks in order of remaining cost: 404, then 505, then 303, then 101 and 202 together.** The owner authorised the full five-seed grid. The ordering is the assistant's and is a scheduling choice, not a scientific one: `run_pilot --only-seeds` runs whole seed blocks, so every phase boundary leaves a **fully crossed** five-arm design at n=1, 2, 3 and 5 rather than a ragged set of arms. Costs 74 generations of wall time on two GPUs against 72 for one undifferentiated launch. | Two reasons the order is worth two extra generations. First, the budget is nearly exhausted and a run that stops partway is a real possibility; under positional sharding that would leave arms at unequal seed counts and support no contrast, whereas a completed block always does. Second, `docs/decisions/powered_design_sizing_2026-08-19.md` sizes three chains per arm against the preregistered 2% threshold at 80% power, so the design meets its own sizing after phase 3 and phase 4 is a strengthening rather than a rescue. Seeds 101 and 202 run together because splitting them costs 15 extra generations: `selection_only` 101 is already complete and neither seed shards evenly alone. **The order is outcome-independent.** Phase order follows how far each chain got before F-025's OOM, which is set by shard scheduling and wall-clock; no chain was kept, dropped or ordered on account of anything it measured, and no seed was chosen after a result was seen. | The run completes all four phases, in which case the ordering leaves no trace in the design and only the 8 pre-existing chains' provenance needs stating. **If it stops early, the paper must report the realised seed count, name the seeds, and state why they were the ones that ran** -- `paper/outcome_contingent_language.md` does not yet cover a partial-grid outcome and would need a case adding. **Outcome, 2026-08-20: executed in full and did not reverse.** All four phases ran, 25 of 25 chains completed, and phase 1 measured 5.07 min per generation -- mid-band. Total 5.96 h across clean launches at the observed rate. Because the run reached phase 4, the ordering left no trace in the design: the realised design is the full preregistered five-seed grid, and the partial-grid language the runbook required if it stopped early was not needed. The ordering's value was insurance that was not called on, which is the correct outcome for insurance and not evidence it was unnecessary |
