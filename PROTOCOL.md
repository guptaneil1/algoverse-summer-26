...
- Framework/model/tokenizer versions: upstream `requirements.txt` at the pinned commit declares only lower bounds — `torch >= 1.3`, `datasets >= 2.14.0`, `accelerate >= 0.12.0` — and leaves `evaluate`, `sentencepiece (!= 0.1.92)`, `protobuf`, `scikit-learn`, `wandb`, `tqdm`, `hydra-core`, `matplotlib`, `nltk`, `textstat`, `mauve-text`, and `seaborn` unpinned. **Upstream installs `transformers` unpinned from Git main** (`pip install git+https://github.com/huggingface/transformers`), so the upstream repository does not by itself determine a reproducible environment. `**Stage A environment freeze (measured 2026-08-17 on the executing host):** `torch 2.8.0+cu128`, `transformers 4.48.3`, `datasets 3.2.0`, `accelerate 1.2.1`; Python `3.12.3`; GPU `NVIDIA GeForce RTX 4090` (compute capability 8.9, bf16 native: True); GPT-2 model/tokenizer revision `607a30d783dfa663caf39e06633721c8d4cfcd7e`. Pins are recorded in `requirements-positive-control.txt` rather than `requirements-lock.txt`, because the latter is generated by `uv export` with `--hash=sha256:` on every entry for `pip install --require-hashes` in CI; hand-added unhashed pins would break every CI job and be erased by the next export.` See `docs/evidence/upstream_pin.md` §4 for the resulting reproducibility hazard and the recommended mitigation.
