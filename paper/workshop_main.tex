...
% The venue style file, loaded WITHOUT [final]: the default is the anonymous
% submission form. [final] is camera-ready and prints author names.
% Drop neurips_2026.sty beside this file and it is
...
(\PilotPrimaryMean{}, $95\%$ CI $[\PilotPrimaryLow,\allowbreak\ \PilotPrimaryHigh]$), and the interval
...
freedom $[\PilotPrimaryLow,\allowbreak\ \PilotPrimaryHigh]$, or \PilotPrimaryPct\% relative. The frozen
...
\item \textbf{Spending at all helps.} Fresh random against the no-rescue control improves the
      outcome by \PilotRandNonePct\%, CI $[\PilotRandNoneLow,\allowbreak\ \PilotRandNoneHigh]$, clearing
...
\item \textbf{Which examples you buy matters more.} Selection-only against fresh random
      improves it by \PilotSelRandPct\%, CI $[\PilotSelRandLow,\allowbreak\ \PilotSelRandHigh]$, at
      identical total volume.
\item \textbf{When you spend does not.} Schedule-only against fresh random gives
      \PilotSchedRandPct\%, CI $[\PilotSchedRandLow,\allowbreak\ \PilotSchedRandHigh]$, an interval
