...
decomposes, and the primary outcome carries signal on one half of it: which under-covered
modes a finite human budget is spent on rather than when it is spent. The decomposition is
not perfectly clean. On the confirmatory tail-retention outcome the timing contrast is
\PilotTailSchedRandMean{}, $95\%$ CI
$[\PilotTailSchedRandLow, \PilotTailSchedRandHigh]$, an interval excluding zero at
\PilotTailSchedRandPct\% of the fresh-random level. Timing therefore does something small
that one preregistered outcome detects and the other does not, and the summary sentence a
reader should carry away is about the primary outcome specifically, not about timing in
general.
