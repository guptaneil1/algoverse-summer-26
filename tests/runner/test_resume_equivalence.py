...
def _seed_generation_zero_checkpoint(full_dir: Path, resume_dir: Path) -> None:
    ...
    Copied as bytes. A crash leaves the file exactly as it was, so a text copy is
    not a faithful simulation — and on Windows it is actively wrong:
    ``save_checkpoint`` writes LF, while ``Path.write_text`` translates to CRLF,
    so the "resumed" run started from a checkpoint that differed byte-for-byte
    from the one the interrupted run had actually written. The hash-equality
    assertion below then failed on a difference this helper introduced.
    """

    (resume_dir / "checkpoints").mkdir(parents=True)
    checkpoint = full_dir / "checkpoints" / "generation_0000.json"
    (resume_dir / "checkpoints" / checkpoint.name).write_bytes(checkpoint.read_bytes())
