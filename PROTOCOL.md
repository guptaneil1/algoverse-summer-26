...
- Exact upstream commit: `feb8511479a2e2dc868e1caf3f63cb99f1fcc746` (default branch `main`, committed 2026-03-13T20:43:11Z, message "Update README.md to reflect EMNLP 2025 acceptance and correct citation format"). Retrieved from the GitHub API on 2026-08-15; full verification record in `docs/evidence/upstream_pin.md`. Upstream license: MIT.

### Initial environment identifiers

- Python (this scaffold's dev environment): `3.12.1`; `pyproject.toml` requires `>=3.10`.
- Framework/model/tokenizer versions: upstream `requirements.txt` at the pinned commit declares only lower bounds — `torch >= 1.3`, `datasets >= 2.14.0`, `accelerate >= 0.12.0` — and leaves `evaluate`, `sentencepiece (!= 0.1.92)`, `protobuf`, `scikit-learn`, `wandb`, `tqdm`, `hydra-core`, `matplotlib`, `nltk`, `textstat`, `mauve-text`, and `seaborn` unpinned. **Upstream installs `transformers` unpinned from Git main** (`pip install git+https://github.com/huggingface/transformers`), so the upstream repository does not by itself determine a reproducible environment. `TODO(khantushig): after the first successful Stage A install, record the resolved torch/transformers/datasets versions and the GPT-2 tokenizer revision actually used, then freeze them into requirements-lock.txt.` See `docs/evidence/upstream_pin.md` §4 for the resulting reproducibility hazard and the recommended mitigation.
