...
### F-026 — resume re-ran the generation it was interrupted in, and upstream refuses a non-empty output directory (2026-08-19)

| Field | Value |
|---|---|
| Stage | v2 grid, second launch. Both shards ran their full chain list and exited 1; 17 of 25 chains failed, 8 completed |
| Observation | Every failure is the same error, from the same line: `ValueError: Output directory (/workspace/v2/<arm>/seed<n>/upstream/<g>/model) already exists and is not empty. Use --overwrite_output_dir to overcome.` raised at `/workspace/model_collapse/src/train.py:313`, before any CUDA allocation. Failures are spread across all five arms -- `no_rescue` 3, `random` 5, `schedule_only` 4, `selection_only` 2, `joint` 3 -- so nothing arm-specific and nothing to do with P-011 |
| Cause | `save_checkpoint` runs at the **end** of a generation, after train, decode, evaluate and allocate. Resume therefore sets `start = checkpoint["generation"] + 1`, which is exactly the generation an interruption was in the middle of. That generation's `upstream/<g>/model` was already populated by the attempt that died. `real_train_step` never passes `--overwrite_output_dir` -- the flag appears nowhere in this repository -- so upstream raises rather than writing into it. **Resume only worked if the interruption landed in the window between the checkpoint write and the next generation's first write**, which is a fraction of a second in a five-minute generation |
| Where the stale directories came from | The first v2 launch, killed by F-025's OOM. That run left 17 chains interrupted mid-generation. The relaunch deliberately kept `/workspace/v2` in order to preserve completed chains, and so inherited all 17 |
| Why the shards exited quickly | The failure is an argument-validation error, so each affected chain died in seconds. `run_pilot.py` catches per-chain exceptions and continues, so both shards burned through their remaining lists and returned 1 at the end. Shard 0: 4 complete, 9 failed, 2.58 h -- the wall time is the four real chains, not the nine failures |
| Correction to F-025 | F-025 states "Nothing scientific was lost: completed chains are skipped on relaunch and **interrupted ones resume from their last checkpoint**". The first clause holds. The second was false at the time it was written, and this is the failure it produced. Appended rather than edited, per the log's rule |
| Correction to a reported count | The `status.sh` line `complete : 10 / 25` was wrong twice over. `grep 'complete,'` also matches `already complete, skipping`, and the glob `/workspace/v2_shard*.log` still matched `v2_shard2.log` and `v2_shard3.log` from the abandoned four-shard launch. The authoritative count is the 8 `chain_result.json` files on disk |
| Not disk | `df -h /workspace` reports 75% used with 537 T free on a 2.1 P network volume. The leading hypothesis was wrong |
| Not memory | The error is a `ValueError` from upstream's argument validation, raised before the model loads. F-025's fix held |
| Fixed | `runner/real_chain` removes `gen_dir / "model"` before training whenever it exists, so a generation being run owns its output directory. Discarding those weights loses nothing: every generation trains from `model.identifier`, not from its predecessor's checkpoint, so no later step reads them -- which is also why `docs/RUNBOOK_V2_CORRECTED_GRID.md` already prunes model directories from completed chains to reclaim disk. `tests/runner/test_resume_clears_stale_generation.py` pins it; 2 of its 3 tests fail if the fix is reverted, verified by reverting it |
| Immediate workaround, no pull needed | For each chain without a `chain_result.json`, `rm -rf` its `upstream/*/model` directories. Do not delete anything else under `upstream/<g>/`: that directory also holds the decoded corpus the next generation trains on, and the checkpoint references it |
| Why nothing caught it | `tests/runner/test_real_chain.py::test_resume_continues_rather_than_restarting` covers exactly this path and passes, because it runs under `dry_run`, which spawns no subprocess and so never creates a `model/` directory to collide with. Fifth enumerated boundary of the same kind, after F-017 (subprocess environment), F-020 (score-dependent budget matching), F-022 (anything consuming the decoded corpus) and F-025 (concurrent shards on real hardware) |
| Scientific consequence | None. No chain produced a wrong number; 17 produced no number at all. The 8 chains that completed are unaffected and are the run's first evidence that both budget axes hold -- see the note below |
| What the 8 completed chains show | Realised total optimizer tokens are **16,678,912 at every one of the eight, across all five arms** -- a 0.0000% spread where F-021 measured 2.26%. Human spend across spending arms runs 749,827 to 749,882, a 0.0073% spread, with `no_rescue` at exactly 0; `joint` consumes 749,827 against the 674,193 that made it invalid in F-020. P-011 and the F-020 fix are validated on real chains. This is the first execution in which `PROTOCOL.md` §4 holds on both axes |
| Lesson | The resume contract was asserted in three documents and one test, and the test could not exercise it. A checkpoint written at the end of a unit of work implies the unit is re-entrant, and nothing had ever re-entered one. The two failures the project has paid for on this pod -- F-025 and F-026 -- are both properties that only exist when a real process is interrupted, which is the one thing no dry run and no CPU test can produce |

### F-026a — the corrected grid completed; both axes hold and the primary contrast is a null (2026-08-20)

| Field | Value |
|---|---|
| Relation to F-026 | **Resolution, appended not edited.** F-026's cause, fix and lesson stand. This records what the fix bought and closes the open thread F-020, F-021 and F-021a left |
| What ran | The v2 grid completed to 25 of 25 chains in four seed-block phases (`DECISIONS.md` P-012): 404, then 505, then 303, then 101 and 202 together. Zero chain failures across all four. The F-026 fix and `--only-seeds` were in the tree for every phase |
| Both axes hold | Human spread across spending arms **0.0381%** against 0.2000% permitted; total optimizer tokens **identical at 16,678,912 for every chain in every arm**, a 0.0000% spread. `run_pilot --check-only` exits 0. F-020 measured 10.1070% on the first axis and F-021 measured 2.2564% on the second |
| Certification | **25 `valid_with_limitation`, 0 `invalid`**, read from the per-chain report in `validation.json` rather than from an aggregate exit code -- which is the specific error F-021 recorded. `validate_run.py` exits 1, the *limited* code (F-024) |
| The result | The preregistered primary contrast is **admissible and null**. Joint minus the strongest eligible non-joint baseline (`selection_only`) is **+0.01026, 95% CI [-0.00916, +0.02968]**, +0.45% relative, against a practical equivalence region of +/-0.05073. The interval lies wholly inside that region. `CLAIMS.md` C-002 is **not supported** -- by its own falsification clause, and now on evidence rather than on a failed precondition |
| P-011 validated | Displacement was accepted unvalidated: "no chain has run under it". 25 chains have now run under it and total optimizer tokens are identical across arms to the token. The confound F-021 measured is closed by construction, and the control arm trains on exactly the volume every spending arm does |
| F-021a answered | F-021a asked whether fixing F-020 would also close the total-token axis and answered no, because the mechanism was untouched by it. Correct: the axis closed because P-011 changed the assembly rule, not because F-020 was fixed. Both were needed and neither sufficed alone |
| The mechanism is still not diagnosed | F-021a's open question -- whether the additive rule's divergence came from block packing or from divergent generated text -- remains open and is now unanswerable from these artifacts, because displacement removes the divergence rather than explaining it. Recorded as a limitation in the paper rather than quietly dropped |
| Cost | 5.96 h of launches in which every chain finished, 9.56 h counting the attempts F-025 and F-026 ended. Roughly $18 and $29 respectively at the observed $3/hour. Throughput 5.07 min per generation on 2x RTX 4090. All read from `wall_seconds`, never from terminal timestamps -- F-020a |
| A wall-time trap this created | The grid was assembled from four sequential phases, so its wall time is the **sum of per-launch maxima**, not the maximum over all shard summaries. A single max returns 3.35 h, which is phase 4 and not the run. `wall_hours` in `scripts/generate_pilot_outputs.py` computes it correctly and its docstring says why; the previous single-max form was right only because every prior run was one launch |
| Analysis change made after the run | `generate_pilot_outputs.py` carried a hardcoded refusal to compute the primary contrast, naming F-020. That was correct for the run in front of it and would have been silently wrong for this one. It now measures both axes from the chains and gates on them. Reported in `docs/runs/primary_pilot_v2_2026-08-20_results.md` rather than left implicit, because it is the difference between a script that refuses a contrast and one that computes it |
| A threshold inconsistency found while wiring this | Three conventions existed for one frozen quantity: `effect_threshold_review_2026-08-19.md` and `powered_design_sizing_2026-08-19.md` both compute the 2% practical threshold against the fresh-random mean, and the generator computed it against the strongest baseline. Corrected to the documented convention. **The verdict is identical under both** (+/-0.05073 and +/-0.04586; the interval reaches 0.0297), and both are emitted as macros so a reader can check. Recorded rather than quietly fixed because it is a threshold denominator touched after primary outcomes were open, which U-006 exists to constrain |
| Scientific consequence | Material and positive. The project now has a valid primary result, from the execution in which its own fairness constraint holds on both axes: a preregistered null with an interval inside the equivalence region, plus three admissible secondary contrasts that locate the effect -- spending helps 4.04%, targeting helps a further 9.59%, timing 0.41% with an interval containing zero |
| Lesson | Two infrastructure defects, F-025 and F-026, cost roughly 3.6 h of pod time and no science: both stopped chains from starting and neither corrupted a chain that finished. The defects that cost science were the quiet ones -- a guard checking half a constraint (F-021), a reconciliation that did not bind (F-020), a documented exit code inverted against its implementation (F-024). Loud failures are cheap. The expensive ones pass |

### F-024a — the inverted exit codes survived in a fifth place, the certificate template (2026-08-20)

| Field | Value |
|---|---|
| Relation to F-024 | **Recurrence, appended not edited.** F-024 corrected the docstring, `Makefile` and both runbooks. It listed four locations and fixed four. There was a fifth |
| Observation | `docs/VALIDITY_CERTIFICATE_TEMPLATE.md` §1 read `Exit code (0 valid, 2 limitation, 1 invalid)` --- F-024's inverted mapping, unchanged, in the document a certifier fills in while deciding whether a run is valid |
| Why it matters more here than in the runbooks | The runbook version misleads an operator mid-run, who has other signals. This version misleads the **certifier**, whose whole function is to be the independent check, and who is reading the template precisely because they did not operate the run and have no other signal. A certifier following it would record a `valid_with_limitation` run as invalid, or an invalid one as valid, and sign the result |
| Cause | The same one F-024 records: the mapping was written down in more places than the fix enumerated. F-024 found four by grepping for the phrasing it happened to remember; this copy words the mapping differently (`2 limitation` rather than `2 valid_with_limitation`) and the grep missed it |
| Fixed | Corrected in place, with the row now naming F-024 and stating that it carried the inverted mapping until 2026-08-20 so a certifier who read the old version can tell |
| Found by | Drafting the certificate evidence pack for the corrected grid, which meant reading the template line by line rather than grepping it |
| Scientific consequence | None. No certificate has been issued against this template, so no run was misclassified by it. The exposure was entirely forward-looking, which is also why it survived: nothing had exercised the document |
| Lesson | A fix that enumerates locations is only as complete as the search that found them, and a search by remembered phrasing finds the phrasings you remember. The durable fix is a single source --- the mapping stated once, in the validator, with every other mention pointing at it rather than restating it. That is not done here and is worth doing before the next document repeats the numbers. Second-order lesson: **documents that nothing exercises do not get corrected by use.** The runbooks were fixed because people run them; the template was wrong for a day longer because nobody had yet had to certify anything |

### F-027 — the paper defined a primary outcome the analysis does not compute (2026-08-20)

| Field | Value |
|---|---|
| Stage | Post-run, during a minimal-edit polish pass over the manuscript. Found by checking section 4's mathematics against the code that produced the numbers, which nothing had previously done |
| Observation | `paper/sections/04_problem.tex` defined the primary chain-level outcome as `A = sum_g w_g (L_g - L_g^ref)` with `sum_g w_g = 1` --- a weighted average of regret against a **frozen per-generation reference**. `scripts/generate_pilot_outputs.auc_regret` computes something else: regret against **the chain's own generation-0 value**, integrated by the **trapezoid rule**, **unnormalised** |
| Magnitude | Three independent differences, and they do not cancel. The paper's `L_g^ref` is a per-generation reference NLL **that appears in no artifact** --- `chain_result.json` metrics carry only `generation`, `human_nll` and `tail_retention`, and `reference_mode_scores.json` holds generation-0 *mode* scores, not a per-generation human-NLL reference. So the paper's formula is **not computable from the released data at all**. Separately, the code's trapezoid weights sum to `G-1 = 9` rather than to 1, so a reader who normalised them would obtain **0.2948** where the paper reports **2.65316**, a factor of exactly nine |
| Consequence | A reviewer implementing the stated definition cannot reproduce a single reported number, and would conclude the results are wrong rather than that the definition is. Every AUC figure in the paper --- the per-arm table, all six contrasts, the variance estimate, the equivalence verdict --- is computed by the code's definition. **No reported number changes**; what was wrong was the sentence describing what those numbers are |
| Not a results defect | The measured quantity is well defined, was computed identically for both grids, and is the same quantity `reproduce_pilot_table.py` independently recomputes. The paired contrast, the interval and the equivalence verdict all stand |
| Which side was corrected, and why | **The paper was changed to describe the code, not the reverse.** The code's definition produced every result on record and predates the outcomes; rewriting it now would change published numbers after outcomes are open, which is the thing this project's freezes exist to prevent. The prose was aspirational preregistration-era text that was never reconciled against the implementation |
| The corrected definition, and why it is the better estimand | Regret is measured within-chain against the chain's own generation 0, so `A` depends on no cross-arm or externally frozen baseline and a paired contrast inherits no variance from a shared reference. That is a real methodological virtue of the implemented quantity and section 4 now states it rather than leaving it implicit. The area is left unnormalised, which is inconsequential because the practical-effect threshold is applied relatively |
| Also present in a second manuscript | The identical formula appears in a separate draft of this paper maintained outside the repository. That draft is not covered by this repository's tests and carries the defect uncorrected. Recorded here because the fix has to be applied there by hand |
| Why nothing caught it | Every guard the project built checks that numbers **trace to artifacts**: no bare decimals in prose, macros defined, digests recorded, values independently recomputable. None checks that the paper's **description of a quantity** matches the function computing it. Prose and mathematics were audited for provenance and never for agreement with the implementation |
| Fixed | `04_problem.tex` now defines the trapezoid-of-own-generation-0 area explicitly, states both deliberate properties, and drops the stale closing sentence claiming the section describes only a planned experiment |
| Lesson | Fifth instance of the shape F-016, F-018, F-020 and F-024 share, and the first to appear in the mathematics rather than in a guard. The project's defence against drift is that prose cites generated macros --- which makes a *number* impossible to mistype and does nothing about a *definition*. A displayed equation is prose as far as every existing check is concerned. The durable fix is a test that the estimand as written and the estimand as implemented agree; that does not exist and is the obvious next guard to build |

### F-028 — per-generation allocation records were not archived, and the pod is gone (2026-08-20)

| Field | Value |
|---|---|
| Stage | Post-run, while attempting to fill an "allocation behaviour" section in a second draft of the paper |
| Observation | Nothing in `results/runs/primary_pilot_v2_2026-08-20/` records **when** a policy spent its budget or **which modes** it spent on. `chain_result.json` carries chain totals; `run_manifest.json` carries configuration and provenance; the shard summaries carry the same totals again. The per-generation `allocations` list exists only in the chain checkpoints, and the checkpoints were not included in the results tarball |
| Recoverable? | **No.** The checkpoints were on the pod at `/workspace/v2/*/seed*/checkpoints/`. The pod was terminated on 2026-08-20 as the runbook directs. Recovering them means re-running the grid: roughly $18 and six hours |
| Whose omission | The assistant's. The tarball contents were specified in `docs/RUNBOOK_V2_CORRECTED_GRID.md` Part 7 and extended during the run to add `reference_mode_scores.json`; checkpoints were not added and their contents were not checked against what the analysis might later want. The runbook's own disk-pruning step deletes model weights and keeps checkpoints, so the information survived on the pod until the pod did not |
| What it costs the paper | The primary contrast, every secondary contrast, the variance estimate and the certification are untouched --- none reads allocations. What is lost is descriptive, and it is the descriptive fact a reviewer is most likely to ask for: **did the joint policy actually behave differently from selection-only, or did it converge to the same allocation?** If joint simply learned to spend like selection-only, the equivalence is explained trivially and is a statement about the policy rule rather than about allocation strategy. **We cannot distinguish those two readings from the retained artifacts** |
| What can still be said | Both arms consumed 749,827 human tokens, identically, which is consistent with both reaching the same terminal reconciliation ceiling and says nothing about timing or mode targeting. `selection_only` follows a fixed schedule by construction, so if `joint` matched it on the primary outcome *and* the two spent identically, the joint rule's adaptive component contributed nothing at this operating point --- but the antecedent is exactly what is unverifiable |
| Consequence | Recorded as a limitation in the manuscript rather than left as an unexplained absence. A reader who asks the obvious question deserves to be told that the answer was not retained, not to find a section quietly missing |
| Third instance of the same shape | F-021a: "the per-generation batch records that would settle it were not retained." F-023: the corpus record count had to be measured because no artifact declared it. Now allocations. Each time, chain-level aggregates were archived and the per-generation detail that would answer the follow-up question was not |
| Fixed for next time | Nothing to fix in code --- the checkpoints are written correctly and contain the data. The defect is in what the archive step selects. Any future results tarball must include `checkpoints/` (a few KB per chain, against the 162 MB of manifests that *are* archived), and the runbook should say so at the point where the tarball is built rather than leaving the selection to whoever writes the command |
| Lesson | The archive was designed around what the analysis needed **that day**. Every artifact question this project has failed to answer after the fact was answerable while the machine was still running. Cheap per-generation records are worth archiving precisely because the questions that need them arrive after the hardware is gone |
