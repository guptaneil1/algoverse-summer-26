...
## Sentence-level licensing

Claim status governs what may be *asserted*; it does not by itself say which
sentences may be *written*. [`docs/evidence/claim_evidence_matrix.md`](docs/evidence/claim_evidence_matrix.md)
maps each planned abstract sentence to the specific artifact that would license
it, and marks each as writable now, blocked on a named artifact, or
outcome-conditional. As of 2026-08-15, four of twelve planned sentences are
writable, six are blocked, and two are outcome-conditional.

Outcome-conditional sentences must use the precommitted language in
[`docs/outcome_templates.md`](docs/outcome_templates.md) rather than prose
composed after seeing a result.
